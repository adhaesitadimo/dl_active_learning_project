...
from .automodel_for_token_classification_custom import AutoModelForTokenClassificationCustom
...
class SequenceTaggerAuto:
    def __init__(self, auto_model, bpe_tokenizer, idx2tag, tag2idx,
    ...
    def _make_tokens_tensors(self, tokens, max_len):
        ...
        bpe_tokens = prepare_bpe_tokens_for_auto_model(bpe_tokens, max_len=max_len)
    ...
    def _make_label_tensors(self, labels, bpe_masks, max_len):
        ...
        bpe_labels = prepare_bpe_labels_for_auto_model(bpe_labels, max_len=max_len)
    ...
    def predict(self, dataset, evaluate=False, metrics=None):
        ...
        dataloader = DataLoader(dataset,
                                collate_fn=lambda dataset_row: self.generate_tensors_for_prediction(
                                    evaluate, dataset_row),
        ...
        for nb, tensors in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                if type(self._auto_model) is AutoModelForTokenClassificationCustom:
                    logits = self._auto_model(token_ids,
                                              token_type_ids=None,
    ...
    def batch_loss_tensors(self, *tensors):
        ...
        if type(self._auto_model) is AutoModelForTokenClassificationCustom:
            output = self._auto_model(token_ids,
                                      token_type_ids=None,
                                      attention_mask=token_masks,
                                      labels=label_ids,
                                      loss_mask=loss_masks)
    ...
    def save_serialize(self, save_dir_path):
        ...
        torch.save(self._auto_model.state_dict(), os.path.join(save_dir_path, 'pytorch_model.bin'))
        with open(os.path.join(save_dir_path, 'bpe_tokenizer.pckl'), 'wb') as f:
            pickle.dump(self._bpe_tokenizer, f)

        self._auto_model.config.save_pretrained(os.path.join(save_dir_path))
    ...
    @classmethod
    def load_serialized(cls, load_dir_path, auto_model_type):
        with open(os.path.join(load_dir_path, 'sec_parameters.json'), 'r') as f:
            parameters_dict = json.load(f)

        auto_model = auto_model_type.from_pretrained(load_dir_path).cuda()

        with open(os.path.join(load_dir_path, 'bpe_tokenizer.pckl'), 'rb') as f:
            bpe_tokenizer = pickle.load(f)

        return SequenceTaggerAuto(auto_model, bpe_tokenizer,
...
def prepare_bpe_tokens_for_auto_model(tokens, max_len):
    return [['[CLS]'] + list(toks[:max_len - 2]) + ['[SEP]'] for toks in tokens]


def prepare_bpe_labels_for_auto_model(labels, max_len):
