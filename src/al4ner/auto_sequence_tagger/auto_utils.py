...
def make_tag_dict_from_flair_corpus(corpus):
...
def get_model_parameters(model, modeltype, no_decay={'bias', 'gamma', 'beta'}, 
    ...
    if full_finetuning:
        ...
        if modeltype == 'roberta':
            grouped_parameters = (get_parameters_without_decay(model.roberta, no_decay) 
                              + grouped_parameters)
        if 'electra' in modeltype:
            print('electra')
            grouped_parameters = (get_parameters_without_decay(model.electra, no_decay) 
                              + grouped_parameters)
