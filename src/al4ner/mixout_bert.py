...
def replace_dropout_with_mixout(model, lmbd=0.9, prefix=''):
    for i, (layer_name, layer) in enumerate(list(model.named_children())):
        if isinstance(layer, nn.Dropout):
            ...
            if re.match('\.bert\.encoder\.layer\.\d+\..*\.(dense|query|key|value)', mod_name):
                ...
                new_module = MixLinear(layer.in_features, 
                                       layer.out_features, 
                                       bias, 
            ...
            replace_dropout_with_mixout(layer, lmbd=lmbd, prefix=prefix + '.' + layer_name)
